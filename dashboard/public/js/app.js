...
document.addEventListener('alpine:init', () => {
  ...
  // ============================================================
  Alpine.data('reactionRolesSection', () => ({
    groups: [],
    roles: [],
    loading: false,
    saving: false,
    ...
    form: {
      channel_id: '',
      title: 'Chon role cua ban',
      description: 'Bam nut ben duoi de tu nhan role.',
      color: '#6366f1',
      exclusive: false,
      items: [],
    },
    ...
    openEdit(g) {
      ...
      this.form = {
        ...
        items: g.items.map(it => ({
          role_id: it.role_id, label: it.label, emoji: it.emoji || '', style: it.style || 'primary',
        })),
      }
      this.modal = true
    },
    ...
    addItem() {
      if (this.form.items.length >= 25) return
      this.form.items.push({ role_id: '', label: '', emoji: '', style: 'primary' })
    },
    ...
    validate() {
      if (!this.form.channel_id || !this.form.title) {
        this.showToast('Thieu channel_id hoac title', 'red')
        return false
      }
      if (this.form.items.length === 0) {
        this.showToast('Can it nhat 1 role', 'red')
        return false
      }
      for (const it of this.form.items) {
        if (!it.role_id || !it.label) {
          this.showToast('Moi item phai co role + label', 'red')
          return false
        }
      }
      return true
    ...
    async save() {
      ...
      this.saving = true
      try {
        const url = this.editId ? `/reaction-roles/${this.editId}` : '/reaction-roles'
        const method = this.editId ? 'PUT' : 'POST'
        const res = await api(method, url, this.form)
        if (res?.success === false) {
          this.showToast(res.error || 'Loi', 'red')
        } else {
          this.showToast('Da luu va post len Discord ✓', 'green')
          this.modal = false
          await this.load()
        }
      } catch (err) {
    ...
    async remove(g) {
      if (!confirm(`Xoa group "${g.title}"? Message tren Discord cung se bi xoa.`)) return
      const res = await api('DELETE', `/reaction-roles/${g.id}`)
      if (res?.success) {
    ...
    styleColor(style) {
      return { primary: '#5865f2', secondary: '#4f545c', success: '#43b581', danger: '#f04747' }[style] || '#5865f2'
    },
